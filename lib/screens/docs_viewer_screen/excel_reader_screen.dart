...
class ExcelReaderScreen extends StatefulWidget {
  ...
  const ExcelReaderScreen({Key? key, required this.filePath, required this.fileName}) : super(key: key);
...
class ExcelReaderScreenState extends State<ExcelReaderScreen> {
                        ...
                        child: SingleChildScrollView(
                          child: SingleChildScrollView(
                            ...
                            child: DataTable(
                                ...
                                columns: List.generate(tableRowsAt[0].length,
                                        (columnIndex) => DataColumn(label: Container(
                                        alignment: Alignment.center,
                                        child: Text("${getColumnLabel(columnIndex)}",textAlign: TextAlign.center,)))),
                                        ...
                                        cells: List.generate(
                                            tableRowsAt[0].length,
                                                ...
                                                Container(
                                                  ...
                                                  alignment: Alignment.center,
                                                  child: Text(
                                                    "${tableRowsAt[index][rowIndex]?.value ?? ""}",textAlign: TextAlign.center,),
                                                )))))),
                          ),
                        ),
                      ),
  ...
  String getColumnLabel(int index) {
    ...
    while (index >= 0) {
      result = String.fromCharCode('A'.codeUnitAt(0) + (index % alphabetSize)) + result;
